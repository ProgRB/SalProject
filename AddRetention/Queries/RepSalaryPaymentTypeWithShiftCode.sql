declare
    ...
    p_payment_type_ids salary.number_collection_type := :p_payment_type_ids;
begin
    select
        payment_type_id bulk collect into exist_payments
    from
        SALARY.report_group
        join SALARY.report_setting using (report_group_id)
    where group_code='3004' and payment_type_id member of p_payment_type_ids;
    
    select count(*) into k 
    from  SALARY.subdiv_for_close 
    ...
    q'[with st as 
        ...
        order by "Месяц", "Подр.", "Таб.№", "Заказ"' using :p_subdiv_id, p_date_begin, p_date_end, p_date_begin, p_date_end, p_date_end, p_date_begin, t;
end;